...
static int usbasp_initialize(PROGRAMMER * pgm, AVRPART * p)
{
  ...
  pdata->sck_3mhz = ((pdata->capabilities & (USBASP_CAP_3MHZ<<24)) != 0) ? 1 :0;	//support for 3 MHz SCK in UsbAsp-flash firmware
  pdata->s5x = ((pdata->capabilities & (USBASP_CAP_S5x<<16)) != 0) ? 1 :0;	//support for AT89S5x and other features of UsbAsp-flash_89x52 firmware
...
static int usbasp_spi_program_enable(PROGRAMMER * pgm, AVRPART * p)
{
  ...
  if (((nbytes != 2) && (PDATA(pgm)->s5x)) || ((nbytes != 1) && !(PDATA(pgm)->s5x)) || (res[0] != 0)) {
  ...
  if(PDATA(pgm)->s5x){
	int i=0;
	unsigned long response_sck=0;
	for (i = 0; i < sizeof(usbaspSCKoptions) / sizeof(usbaspSCKoptions[0]); i++) 
		if(usbaspSCKoptions[i].id==res[1])
			response_sck=usbaspSCKoptions[i].frequency;
	
	avrdude_message(MSG_INFO, "%s: target responded with SCK frequency: %d Hz (possible auto slowing down)\n",progname, response_sck);
  }
...
static int usbasp_spi_set_sck_period(PROGRAMMER *pgm, double sckperiod)
{
  ...
  if (sckperiod == 0) {
	...
	int i;
	if((PDATA(pgm)->s5x)) {
	  avrdude_message(MSG_NOTICE2, "%s: connected USBasp is capable of 6 MHz SCK\n",progname);
	  i=0;
	} else if((PDATA(pgm)->sck_3mhz)) {
	  avrdude_message(MSG_NOTICE2, "%s: connected USBasp is capable of 3 MHz SCK\n",progname);
	  i=1;
	} else {
	  avrdude_message(MSG_NOTICE2, "%s: connected USBasp is cabable of 1.5 MHz SCK\n",progname);
	  i=2;
	}
    if (sckfreq >= usbaspSCKoptions[i].frequency) {
      ...
      for (i = 0; i < sizeof(usbaspSCKoptions) / sizeof(usbaspSCKoptions[0]); i++) {
		  if(((PDATA(pgm)->s5x)==0) && (usbaspSCKoptions[i].frequency == 6000000) ) 
			  /* ommit 6 MHz if not capable */ ;
	  	  else if(((PDATA(pgm)->sck_3mhz)==0) && (usbaspSCKoptions[i].frequency == 3000000) ) 
